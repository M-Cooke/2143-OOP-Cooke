"""
Added SprintToFinish, Aggressive, and Cautious strategies
Micah Cooke
...
class Player(object):
    ...
    @Description: Unless the roll equals 0, the player will continue
    to roll until they reach the target score.
    @Params: None 
    ...
    def SprintToFinish(self):
    	Score = 0
    	NumRolls = 0
    	while self.TotalScore + score < self.TargetScore:
    		NumRolls += 1
    		roll = self.pig.Roll()
    		if roll == 0:
    			break
    ...
    """
    @Method: Aggressive
    @Description: The player will roll a random number of rolls between
    6 and 9, which is roughly 3 more rolls than the average. 
    @Params: None 
    @Returns: (Score, NumRolls)
    """                
    def Aggressive(self):
        Score = 0
        NumRolls = 0
        for i in range(random.randint(6,self.Strategies['Aggressive'])):
        	if self.TotalScore + Score < self.TargetScore:
        		NumRolls += 1
        		roll = self.pig.Roll()
        		if roll == 0:
    ...
    @Method: Cautious
    @Description: The player will roll a random number of rolls between 
    3 and 6, which is roughly 3 less rolls than the average.
    @Params: None 
    @Returns: (Score, NumRolls)
    """    
    def Cautious(self):
